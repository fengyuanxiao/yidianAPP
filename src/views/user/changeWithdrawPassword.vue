<template>
  <div class="commonForm" id="changeWithdrawPassword">
    <user-header :title="'创建/修改提现密码'"></user-header>
    <div class="loginForm">
      <group title>
      ...
      <group title>
        <x-input title="新密码" v-model.trim="userInfo.password" type="password" placeholder="请输入新登录密码">
          <!-- <span class="font-suo" slot="label"></span> -->
        </x-input>
      </group>
      <group title>
        <x-input title="确认密码" v-model.trim="userInfo.repassword" type="password" placeholder="请输入确认密码">
          <!-- <span class="font-querenmima" slot="label"></span> -->
        </x-input>
      </group>
      <x-button type="primary" @click.native="updateDraw" class="loginbtn">立即修改</x-button>
