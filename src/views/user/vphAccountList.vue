<template>
  <div class="vphAccountList">
    <user-header :title="'唯品会买号信息'"></user-header>
    <group v-for="item in vphList" :key="item.value">
    ...
    <x-button class="addAccount" :link="'/h5/user/addVphAccount'">+ 新增一个唯品会账号</x-button>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    ...
    goUpdate(item) {
      if (item.status == 3) {
        this.$router.push("/h5/user/addVphAccount?id=" + item.id);
      }
    },
    async getVphList() {
      ...
      this.vphList = result.data;
      if (this.vphList.status == 0) {
        this.isCertified = "未绑定";
      } else if (this.vphList.status == 1) {
        this.isCertified = "已审核";
      } else if (this.vphList.status == 2) {
        this.isCertified = "审核中";
      } else if (this.vphList.status == 3) {
