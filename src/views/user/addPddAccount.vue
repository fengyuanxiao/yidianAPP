    ...
    <div class="addUser">
      ...
      <div class="userInfoForm">
        ...
        <div class="mui-submite" style="margin:40px 10px">
          <x-button type="primary" style="border-radius:30px;" @click.native="bandPdd">提交</x-button>
        </div>
      </div>
    </div>
  </div>
      ...
      uploadPhotoes: [
        {
          ...
          title: "会员中心截图",
          showExample: false,
          exampleImg: require("@/assets/img/pdd/pinduoduo1.png")
        },
        {
          value: 2,
          title: "购物订单截图",
          showExample: false,
          exampleImg: require("@/assets/img/pdd/pinduoduo2.png")
        }
      ]
    };
  },
  ...
  methods: {
    ...
    async bandPdd() {
      if (this.userInfo.Account === "") {
        return this.$vux.toast.text("请输入拼多多用户名");
      }
      ...
      if (this.images[1].length === 0) {
        return this.$vux.toast.text("请输入拼多多订单截图");
      }
      const reuslt = await this.axios.post(
        this.$route.query.id
          ? "/api/index/updatepdd_bind"
          : "/api/index/pdd_bind",
