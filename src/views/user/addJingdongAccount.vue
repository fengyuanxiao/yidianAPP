    ...
    <div class="addUser">
      <div class="warning">
        ...
        <div class="pad5 bgwhite" style="border-bottom:4px solid #e0e0e0;">
          <p>
            ...
            >绑定账号必须实名认证、信誉铜牌以上，京享值要求1000以上，半月前有购买京东自营商品订单的账号。</span>
          </p>
          <p style="clear:both;">
            <span style="float:left;">2、</span>
            <span
              style="float:left;word-break: break-all;width:90%;"
            >京东可绑定2个买号，每个买号要求绑定不同的收货信息（收货人姓名，地址，联系电话均要求不同并且真实有效，可以联系上买手本人同事或者是亲戚</span>
          </p>
      ...
      <div class="userInfoForm">
        ...
        <div class="mui-submite" style="margin:40px 10px">
          <x-button type="primary" style="border-radius:30px;" @click.native="bandJingdong">提交</x-button>
        </div>
      </div>
    </div>
  </div>
      ...
      uploadPhotoes: [
        {
          ...
          title: "个人中心截图",
          showExample: false,
          exampleImg: require("@/assets/img/jd/jd1.png")
        },
        {
          value: 2,
          title: "账户设置截图",
          showExample: false,
          exampleImg: require("@/assets/img/jd/jd2.png")
        },
        {
          value: 3,
          title: "实名认证截图",
          showExample: false,
          exampleImg: require("@/assets/img/jd/jd3.png")
        },
        {
          value: 4,
          title: "plus会员截图",
          showExample: false,
          exampleImg: require("@/assets/img/jd/plus.png")
        }
      ]
    };
  },
  ...
  methods: {
    ...
    async bandJingdong() {
      if (this.userInfo.Account === "") {
        return this.$vux.toast.text("请输入京东用户名");
      }
      ...
      if (this.images[0].length === 0) {
        return this.$vux.toast.text("请上传个人中心截图");
      }
      if (this.images[1].length === 0) {
        return this.$vux.toast.text("请上传账户设置截图");
      }
      if (this.images[2].length === 0) {
        return this.$vux.toast.text("请上传个实名认证截图");
      }
      if (this.images[3].length === 0) {
        return this.$vux.toast.text("请上传plus会员截图");
      }
      const reuslt = await this.axios.post(
        this.$route.query.id
          ? "/api/index/updatejd_bind"
          : "/api/index/jd_bind",
