    ...
    <template v-if="orderInfo">
      ...
      <div class="plan-box task-plan" style="margin-bottom:10px;-webkit-touch-callout:none;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;">
        ...
        <p class="task-plan-list">
          <span>任务类型</span>
          <span style="color:#FF9642">{{orderInfo.new_type ==1 ? '垫付任务' :orderInfo.new_type ==2?'回访任务' :orderInfo.new_type ==3 ? '图文任务' :orderInfo.new_type ==4 ?'问答任务':'浏览任务'}}({{orderInfo.tasktype_itemname =='手机天猫' ? '手机淘宝' : orderInfo.tasktype_itemname}})</span>
        </p>
        <p class="task-plan-list">
          <span>是否包邮</span>
        ...
        <p class="task-plan-list" >
          ...
          <span v-if="orderInfo.taskInfo.paychannel !==0">
            <template v-for="item in orderInfo.taskInfo.paychannel">可以使用{{item+'、'}}</template>
          </span>
        </p>
